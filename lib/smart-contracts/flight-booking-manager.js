...
class FlightBookingManager extends ResourceManager {
    constructor() {
    ...
    async isSeatAvailable(ctx, txId) {
        let seatOwner = await this.getValue(ctx, txId, "seatOwner");
    ...
    async changeSeatPrice(ctx, txId, newPrice) {
        await this.setValue(ctx, txId, "seatPrice", newPrice);
    }

    async querySeatPrice(ctx, txId) {
        let defaultPrice = 500;
        let price = await this.getValue(ctx, txId, "seatPrice");
    ...
    async queryClientBalance(ctx, txId) {
        ...
        let balance = await this.getValue(ctx, txId, variableName);
    ...
    async addToClientBalance(ctx, txId, amountToAdd) {
        ...
        await this.setValue(ctx, txId, variableName, newBalance);
    }
    ...
    async bookSeat(ctx, txId) {
        ...
        if (isSeatAvailable == true) {
            await this.setValue(ctx, txId, "seatOwner", ctx.clientIdentity.getID());
 ...
 */
    async hasReservation(ctx, txId) {
        let owner = await this.getValue(ctx, txId, "seatOwner");

        return owner && owner == ctx.clientIdentity.getID();
    }

    async endFlight(ctx, txId) {
        await this.setValue(ctx, txId, "seatOwner", null);
    }
