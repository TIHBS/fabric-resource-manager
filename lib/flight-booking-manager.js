...
class FlightBookingManager extends Contract {
    ...
    async isSeatAvailable(ctx, txId, seatNumber, tm) {
        let seatsCount = await this.querySeatsCount(ctx, txId, tm);
        let reserved = await this.queryBookedSeatsCount(ctx, txId, tm);
    ...
    async isSeatBookedByClient(ctx, txId, seatNumber, tm) {
        let bookedSeats = await this.queryNextAvailableSeat(ctx, txId, tm);
    ...
    async hasReservation(ctx, txId, tm) {
        ...
        for (let i = 0; i < reservations; i++) {
            if (await this.isSeatBookedByClient(ctx, txId, i, tm)) {
                return true;
            }
        }
     ...
     * @returns {Promise<void>}
     */
    async changeSeatCount(ctx, txId, newCount, tm) {
        if (newCount < 0) {
    ...
    async queryBookedSeatsCount(ctx, txId, tm) {
        return +(await this._queryVariable(ctx, txId, 'bookedSeats', '0', tm));
    }
     ...
     * @returns {Promise<void>}
     */
    async changeSeatPrice(ctx, txId, newPrice, tm) {
    ...
    async querySeatPrice(ctx, txId, tm) {
        return +(await this._queryVariable(ctx, txId, 'seatCount', '500', tm));
    }
     ...
     * @returns {Promise<void>}
     */
    async addToClientBalance(ctx, txId, amountToAdd, tm) {
        if (amountToAdd < 0) {
     ...
     * @returns {Promise<void>}
     */
    async bookSeat(ctx, txId, seatNumber, tm) {
        let isSeatAvailable = await this.isSeatAvailable(ctx, txId, seatNumber, tm);
        ...
        if (isSeatAvailable) {
            ...
            if (!await (new ResourceManager()).setValue(ctx, txId, 'bookedSeats', (seatNumber + 1).toString(), tm)) {
    ...
    async endFlight(ctx, txId, tm) {
        ...
        if (!await (new ResourceManager()).setValue(ctx, txId, 'bookedSeats', '0', tm)) {
            throw new Error('Cannot change seat status. Data item is locked by another transaction!');
        }
    }
}
