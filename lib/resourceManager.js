...
class ResourceManager extends Contract {
     ...
     * @private
     */
    async _ensureStarted(ctx, txId, tm) {
        ...
        if (!txDetails) {
            ...
            txDetails = TxDetails.createInstance(txId, ctx.clientIdentity.getID(), timeout, tm);
     ...
     * @returns {Promise<void>}
     */
    async _requireTxStarted(ctx, txId, tm) {
        const isStarted = await this._ensureStarted(ctx, txId, tm);
    ...
    async _acquireLock(ctx, variableName, txId, lockType) {
        ...
        if ((lockType === LockType.READ_LOCK && (variable.getWriteLockHolder() === null || variable.getWriteLockHolder() === undefined)) ||
        ...
        if (variable.getWriteLockHolder() !== null && variable.getWriteLockHolder() !== undefined) {
    ...
    static _isVariableLockedByOtherTx(variable, txId) {
        return (variable.getWriteLockHolder() !== null && variable.getWriteLockHolder() !== undefined && variable.getWriteLockHolder() !== txId) ||
     ...
     */
    async setValue(ctx, txId, variableName, variableValue, tm) {
        await this._requireTxStarted(ctx, txId, tm);
     ...
     */
    async getValue(ctx, txId, variableName, tm) {
        await this._requireTxStarted(ctx, txId, tm);
        await this._requireSameOwner(ctx, txId);

        let { isSuccessful} = await this._acquireLock(ctx, variableName, txId, LockType.READ_LOCK);
    ...
    async prepare(ctx, txId) {
        await this._requireSameTm(ctx, txId);
        ...
        if (tx.getState() === TxState.STARTED) {
            ...
            await ctx.TxDetailsList.updateTxDetails(tx);
            ctx.stub.setEvent('Voted', Buffer.from(stringify({
                owner: tx.getOwner(),
                txId: tx.getId(),
                isYes: 'true'
            })));
