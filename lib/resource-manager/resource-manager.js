...
class ResourceManager extends Contract {
    ...
    async _acquireLock(ctx, variableName, txId, lockType) {
        ...
        if (!variable) {
            variable = Variable.createInstance(variableName, "");
        ...
        if ((lockType === LockType.WRITE_LOCK && variable.getWriteLockHolder() === txId) ||
        ...
        } else if ((lockType === LockType.READ_LOCK && variable.getWriteLockHolder() == null) ||
            ...
            const now = DateTime.fromJSDate(ctx.getDateTimestamp());
            /**
             * @type {TxDetails[]}
             */
            const allHolders = [];
