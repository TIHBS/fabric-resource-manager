...
class ResourceManager extends Contract {
  ...
  async _isWL(ctx, variableName) {
    ...
    if (variable) {
      return variable.getWriteLockHolder() && variable.getWriteLockHolder().length > 0;
    }
  ...
  async _acquireLock(ctx, variableName, txId, lockType) {
    if (await this._canObtainLock(ctx, variableName, txId, lockType)) {
      let {variable, hasSetLock} = await this._lock(ctx, variableName, txId, lockType);
      return {variable, hasSetLock};
    }
    return {variable: null, hasSetLock: false};
  }
  ...
  async _getAllLockedVariables(ctx, txId) {
    return  (await ctx.variableList.getAllVariables()).filter(v => v.getWriteLockHolder() === txId || v.getReadLocks().indexOf(txId) >= 0);
  }
  ...
  async setValue(ctx, txId, variableName, variableValue) {
    ...
    let {variable, hasSetLock} = await this._acquireLock(ctx, variableName, txId, LockType.WRITE_LOCK);

    if (!variable) {
      throw new Error("Cannot lock the variable: '" + variableName + "' for writing!");
    }
    
    // do not update the before image unless this is the first write to this variable
    // in this transaction
    if(hasSetLock == true) {
