...
class ResourceManager extends Contract {
  ...
  async _releaseLocks(ctx, txId) {
    ...
    for (let name of tx.getLockedVariables()) {
      await this._releaseLock(name, txId);
    }

  }

  async _releaseLock(variableName, txId) {
  ...
  async begin(ctx, txId) {
    ...
    let txKey = TxDetails.makeKey([txId]);
    let oldTx = await ctx.TxDetailsList.getTxDetails(txKey);
    ...
    await ctx.stub.setEvent('TransactionStarted',
 ...
 */
  async test(ctx) {
    let key = ctx.stub.createCompositeKey("a", ["b"]);

    await ctx.stub.putState("abc", Buffer.from(JSON.stringify({att1: 1200})));
  }

  async test2(ctx) {
    let key = ctx.stub.createCompositeKey("a", ["b"])
    let raw = await ctx.stub.getState("abc");
